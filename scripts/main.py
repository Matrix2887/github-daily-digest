...
class GitHubTrendingScraper:
    ...
    def get_daily_top10(self, max_retries: int = 3) -> List[Project]:
        ...
        return self._get_trending("daily", max_retries)
    
    def get_weekly_top10(self, max_retries: int = 3) -> List[Project]:
        """获取本周 Stars 增长 Top 10"""
        return self._get_trending("weekly", max_retries)
    
    def _get_trending(self, since: str, max_retries: int = 3) -> List[Project]:
        """获取 Trending 项目"""
        for attempt in range(max_retries):
            try:
                logger.info(f"尝试获取 GitHub Trending ({since}) (第 {attempt + 1} 次)")
                ...
                for i, repo in enumerate(repo_list[:10]):
                    try:
                        ...
                        if today_elem:
                            ...
                            stars_growth = int(today_text) if today_text.isdigit() else 0
                        ...
                        if since == "daily":
                            project.stars_today = stars_growth
                        else:
                            project.stars_weekly = stars_growth
                ...
                logger.info(f"成功获取 {len(projects)} 个 Trending 项目 ({since})")
                return projects
...
class GitHubAPI:
    ...
    def get_most_starred_top10(self, max_retries: int = 3) -> List[Project]:
        """获取 Stars 总数最高的 Top 10 项目"""
        for attempt in range(max_retries):
            try:
                logger.info(f"尝试获取 GitHub Stars 最多的项目 (第 {attempt + 1} 次)")
                ...
                logger.info(f"成功获取 {len(projects)} 个 Stars 最多的项目")
                return projects
                
            except Exception as e:
                logger.error(f"获取 GitHub 数据失败 (第 {attempt + 1} 次): {e}")
                if attempt < max_retries - 1:
                    time.sleep(5)
                else:
                    return []
...
class DailyDigestGenerator:
    ...
    def generate_report(self) -> Optional[str]:
        ...
        try:
            ...
            # 3. 获取本周 Trending Top 10
            logger.info("开始获取本周 Trending Top 10...")
            weekly_projects = self.trending_scraper.get_weekly_top10()
            
            # 4. 获取 Stars 总数最高的 Top 10
            logger.info("开始获取 Stars 最多的 Top 10...")
            most_starred_projects = self.github_api.get_most_starred_top10()
            
            # 5. 生成中文总结
            logger.info("生成中文总结...")
            all_projects = trending_projects + rising_projects + weekly_projects + most_starred_projects
    ...
    def _generate_html_report(
        self, 
        trending: List[Project], 
        rising: List[Project],
        weekly: List[Project],
        most_starred: List[Project]
    ) -> str:
        """生成 HTML 报告"""
        ...
        for i, p in enumerate(weekly, 1):
            ...
            <tr>
                ...
                <td>{p.language}</td>
                <td>⭐ {p.stars:,}</td>
                <td>+{p.stars_weekly:,}</td>
            </tr>
            """
        
        # Most Starred 表格
        most_starred_rows = ""
        for i, p in enumerate(most_starred, 1):
            most_starred_rows += f"""
            <tr>
                <td>{i}</td>
                <td><a href="{p.url}" target="_blank">{p.name}</a></td>
...
<tr><th>#</th><th>项目</th><th>简介</th><th>语言</th><th>Stars</th><th>昨日增长</th></tr>
{trending_rows}
</table>
...
<h3>📅 本周订阅增长 Top 10</h3>
<table border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:100%;">
<tr><th>#</th><th>项目</th><th>简介</th><th>语言</th><th>Stars</th><th>本周增长</th></tr>
{weekly_rows}
</table>

<h3>⭐ Stars 总数最高 Top 10</h3>
<table border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:100%;">
<tr><th>#</th><th>项目</th><th>简介</th><th>语言</th><th>Stars</th><th>Forks</th></tr>
{most_starred_rows}
</table>

<p style="color:#666;font-size:12px;margin-top:30px;">数据来源: GitHub Trending + OssInsight API + GitHub API | 自动生成于 {datetime.now().strftime('%Y-%m-%d %H:%M')}</p>
</body>
</html>"""
